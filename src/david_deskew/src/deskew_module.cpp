...
DavidDeskewer::DavidDeskewer(ros::NodeHandle &node_handle) : nh_(node_handle) {
  // 订阅IMU数据
  sub_imu_ = nh_.subscribe<sensor_msgs::Imu>("/imu/raw_data", 50, &DavidDeskewer::ImuHandler, this);

  // 订阅激光数据
  auto laser_topic_name = nh_.param<std::string>("laser_topic_name", "/scan");
  sub_laser_ = nh_.subscribe<sensor_msgs::LaserScan>(laser_topic_name, 5, &DavidDeskewer::LaserScanHandler, this);
  ...
  // 创建单独的线程处理数据
  deskew_thread_ = std::thread(&DavidDeskewer::DeskewThreadFunc, this);
...
DavidDeskewer::~DavidDeskewer() {
  // 设置结束
  is_termination_ = true;
  deskew_data_ch_->send({});
  deskew_thread_.join();

  std::cout << "~DavidDeskewer" << std::endl;
  LOG(INFO) << "~DavidDeskewer";
}

void DavidDeskewer::PublishScan() {
  // 发送LaserScan
...
void DavidDeskewer::ResetSize() {
  laser_cloud_input_.reset(new pcl::PointCloud<PointXYZIT>());
  deskewed_cloud_.reset(new pcl::PointCloud<pcl::PointXYZI>());
}

void DavidDeskewer::ResetParameters() {
  laser_cloud_input_->clear();
  deskewed_cloud_->clear();

  current_imu_pointer_ = 0;
...
void DavidDeskewer::ImuHandler(const sensor_msgs::Imu::ConstPtr &imuMsg) {
  std::lock_guard<std::mutex> lock(imu_lock_);
  imu_data_queue_.push_back(*imuMsg);

  // 防止队列无限增大
  while (imu_data_queue_.size() > 500) {
    imu_data_queue_.pop_front();
  }
}

void DavidDeskewer::DeskewThreadFunc() {
...
void DavidDeskewer::LaserScanHandler(const sensor_msgs::LaserScan::ConstPtr &laser_info_msg) {
  static int cnt = 0;
  // 发送数据
...
bool DavidDeskewer::GetLaserInfo(const sensor_msgs::LaserScan::ConstPtr &laser_info_msg) {
  // 保存前值
  laser_msg_ptr_ = laser_info_msg;

  // 转换到带时间戳的点云
  int size = (int)laser_info_msg->ranges.size();
  laser_cloud_input_->reserve(size);
  ...
  for (auto i = 0; i < size; i++) {
    ...
    laser_cloud_input_->push_back(p);
  ...
  // 时间改为最后一个点的时间
  time_scan_next_ = time_packet_ + laser_cloud_input_->back().time;
...
int DavidDeskewer::DeskewInfo() {
  static int cnt = 0;
  std::lock_guard<std::mutex> lock(imu_lock_);

  // 没有imu数据 或者 最早的imu数据比当前第一个激光点数据新 或者 最新的imu数据比当前最后一个激光点旧
  if (imu_data_queue_.empty() || imu_data_queue_.front().header.stamp.toSec() > time_scan_cur_ ||
      imu_data_queue_.back().header.stamp.toSec() < time_scan_next_) {
    cnt++;
    // 2s输出一次
    if (cnt == 200) {
      cnt = 0;
    ...
    if (imu_data_queue_.front().header.stamp.toSec() > time_scan_cur_) {
      return 2;
    }
  ...
  // ROS_INFO("imu time:%.6f - %.6f", imu_data_queue_.front().header.stamp.toSec(),
  // imu_data_queue_.back().header.stamp.toSec());
  ImuDeskewInfo();
  cnt = 0;
...
void DavidDeskewer::ImuDeskewInfo() {
  is_imu_available_ = false;

  while (!imu_data_queue_.empty()) {
    if (imu_data_queue_.front().header.stamp.toSec() < time_scan_cur_ - 0.01)
      imu_data_queue_.pop_front();
    else
      break;
  }
  ...
  for (int i = 0; i < (int)imu_data_queue_.size(); ++i) {
    sensor_msgs::Imu thisImuMsg = imu_data_queue_[i];
    double currentImuTime = thisImuMsg.header.stamp.toSec();

    if (currentImuTime > time_scan_next_ + 0.01) break;

    if (current_imu_pointer_ == 0) {
      imu_rot_X_[0] = 0;
      imu_rot_Y_[0] = 0;
      imu_rot_Z_[0] = 0;
      imu_time_[0] = currentImuTime;
      ++current_imu_pointer_;
      continue;
    }
    ...
    // integrate rotation
    double timeDiff = currentImuTime - imu_time_[current_imu_pointer_ - 1];
    imu_rot_X_[current_imu_pointer_] = imu_rot_X_[current_imu_pointer_ - 1] + angular_x * timeDiff;
    imu_rot_Y_[current_imu_pointer_] = imu_rot_Y_[current_imu_pointer_ - 1] + angular_y * timeDiff;
    imu_rot_Z_[current_imu_pointer_] = imu_rot_Z_[current_imu_pointer_ - 1] + angular_z * timeDiff;
    imu_time_[current_imu_pointer_] = currentImuTime;
    // ROS_INFO("imu: (%.2f, %.2f, %.2f)", RAD2DEG(imu_rot_X_[current_imu_pointer_]),
    // RAD2DEG(imu_rot_Y_[current_imu_pointer_]), RAD2DEG(imu_rot_Z_[current_imu_pointer_]));
    ++current_imu_pointer_;
  }

  --current_imu_pointer_;
  // ROS_INFO("current_imu_pointer_: %d, (%.2f, %.2f, %.2f)", current_imu_pointer_,
  // RAD2DEG(imu_rot_X_[current_imu_pointer_]), RAD2DEG(imu_rot_Y_[current_imu_pointer_]),
  // RAD2DEG(imu_rot_Z_[current_imu_pointer_]));

  if (current_imu_pointer_ <= 0) return;

  is_imu_available_ = true;
}

void DavidDeskewer::FindRotation(double pointTime, float *rotXCur, float *rotYCur, float *rotZCur) {
  *rotXCur = 0;
  *rotYCur = 0;
  *rotZCur = 0;

  int imuPointerFront = 0;
  while (imuPointerFront < current_imu_pointer_) {
...
pcl::PointXYZI DavidDeskewer::DeskewPoint(pcl::PointXYZI *point, double relTime) {
...
void DavidDeskewer::ProjectPointcloud() {
  int cloudSize = laser_cloud_input_->points.size();

  // int modify_cnt = 0;
  // range image projection
  for (int i = 0; i < cloudSize; ++i) {
    pcl::PointXYZI thisPoint;
    thisPoint.x = laser_cloud_input_->points[i].x;
    thisPoint.y = laser_cloud_input_->points[i].y;
    thisPoint.z = laser_cloud_input_->points[i].z;
    thisPoint.intensity = laser_cloud_input_->points[i].intensity;

    thisPoint = DeskewPoint(&thisPoint, laser_cloud_input_->points[i].time);
...
int main(int argc, char *argv[]) {
  ...
  ros::init(argc, argv, "david_deskew_node");
  ros::NodeHandle nh("~");

  LOG(INFO) << "\033[1;32m Laser deskew node Started.\033[0m";

  DavidDeskewer laser_deskewer(nh);

  // 两个接收订阅
  ros::MultiThreadedSpinner spinner(2);
  spinner.spin();

  std::cout << "david_deskew_node exited!!!" << std::endl;
  LOG(INFO) << "david_deskew_node:-----------------------exited!!";
  return 0;
}
