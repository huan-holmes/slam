...
void convert(const mrpt::graphs::CNetworkOfPoses2DInf& mrpt_graph, mrpt_msgs::NetworkOfPoses& ros_graph) {
  ...
  for (poses_cit_t poses_cit = mrpt_graph.nodes.begin(); poses_cit != mrpt_graph.nodes.end(); ++poses_cit) {
  ...
  for (CNetworkOfPoses2DInf::const_iterator constr_it = constraints.begin(); constr_it != constraints.end();
       ++constr_it) {
...
void convert(const mrpt_msgs::NetworkOfPoses& ros_graph, mrpt::graphs::CNetworkOfPoses2DInf& mrpt_graph) {
  ...
  // fill nodeIDs, positions
  for (nodes_cit_t nodes_cit = ros_graph.nodes.vec.begin(); nodes_cit != ros_graph.nodes.vec.end(); ++nodes_cit) {
    // get the pose
    CPose2D mrpt_pose;
    convert(nodes_cit->pose, mrpt_pose);

    mrpt_graph.nodes.insert(make_pair(static_cast<TNodeID>(nodes_cit->nodeID), mrpt_pose));
  }

  // fill the constraints
  for (constraints_cit_t constr_cit = ros_graph.constraints.begin(); constr_cit != ros_graph.constraints.end();
       ++constr_cit) {
    // constraint ends
    auto constr_ends(
        make_pair(static_cast<TNodeID>(constr_cit->nodeID_from), static_cast<TNodeID>(constr_cit->nodeID_to)));
...
void convert(const mrpt_msgs::NetworkOfPoses& ros_graph, mrpt::graphs::CNetworkOfPoses2DInf_NA& mrpt_graph) {
  ...
  for (nodes_cit_t nodes_cit = ros_graph.nodes.vec.begin(); nodes_cit != ros_graph.nodes.vec.end(); ++nodes_cit) {
    ...
    mrpt_graph.nodes.insert(make_pair(static_cast<TNodeID>(nodes_cit->nodeID), mrpt_node));
  }
