...
geometry_msgs::Pose& mrpt_bridge::convert(const mrpt::math::TPose2D& _src, geometry_msgs::Pose& _des) {
  _des.position.x = _src.x;
  _des.position.y = _src.y;
  _des.position.z = 0;
...
mrpt::poses::CPose3D& mrpt_bridge::convert(const geometry_msgs::Pose& _src, mrpt::poses::CPose3D& _des) {
  mrpt::math::CQuaternionDouble q(_src.orientation.w, _src.orientation.x, _src.orientation.y, _src.orientation.z);
  // Ensure the real part of the quaternion is >=0.
  // It seems ROS does not always ensure it.
  if (q.r() < 0) {
    q.r(-q.r());
