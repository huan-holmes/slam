...
bool convert(const marker_msgs::MarkerDetection& src, const mrpt::poses::CPose3D& pose,
  ...
  for (size_t i = 0; i < src.markers.size(); i++) {
    ...
    measurment.range =
        sqrt(marker.pose.position.x * marker.pose.position.x + marker.pose.position.y * marker.pose.position.y);
