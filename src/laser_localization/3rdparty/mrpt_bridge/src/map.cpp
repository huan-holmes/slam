...
bool convert(const nav_msgs::OccupancyGrid& src, COccupancyGridMap2D& des) {
  if ((src.info.origin.orientation.x != 0) || (src.info.origin.orientation.y != 0) ||
    ...
    printf("!!!!!!!Rotated maps are not supported by mrpt!q:(%.4f, %.4f, %.4f, %.4f)\n", src.info.origin.orientation.x,
           src.info.origin.orientation.y, src.info.origin.orientation.z, src.info.origin.orientation.w);
    // return false;
  }
  ...
  MRPT_END
  printf("--------convert:  %i x %i, %4.3f, %4.3f, %4.3f, %4.3f, r:%4.3f\n", des.getSizeX(), des.getSizeY(),
         des.getXMin(), des.getXMax(), des.getYMin(), des.getYMax(), des.getResolution());
...
const bool MapHdl::loadMap(CMultiMetricMap& _metric_map, const CConfigFile& _config_file, const std::string& _map_file,
                           const std::string& _section_name, bool _debug) {
  ...
  if (_map_file.size() < 3) {
    ...
    std::string mapExt =
        mrpt::system::lowerCase(mrpt::system::extractFileExtension(_map_file, true));  // Ignore possible .gz extensions
