...
int sba::readBundlerFile(const char *filename, SysSBA &sbaout) {
  ...
  vector<vector<Vector4d, Eigen::aligned_allocator<Vector4d> > >
      ptts;  // point tracks - each vector is <camera_index kp_idex u v>
  ...
  int npts = ptps.size();
  int nprjs = 0;
  for (int i = 0; i < npts; i++) nprjs += (int)ptts[i].size();
  ...
  for (int i = 0; i < npts; i++) {
    ...
    for (int j = 0; j < nprjs; j++) {
      ...
      if (cami >= ncams) cout << "*** Cam index exceeds bounds: " << cami << endl;
...
int sba::writeBundlerFile(const char *filename, SysSBA &sbain) {
  ...
  for (i = 0; i < sbain.tracks.size(); i++) {
    // World <x y z>
    outfile << sbain.tracks[i].point(0) << ' ' << sbain.tracks[i].point(1) << ' ' << sbain.tracks[i].point(2) << endl;
    ...
    for (ProjMap::iterator itr = prjs.begin(); itr != prjs.end(); itr++) {
      ...
      outfile << prj.ndi << ' ' << i << ' ' << prj.kp(0) - cx << ' ' << -(prj.kp(1) - cy) << ' ';
    }
...
int sba::ParseBundlerFile(const char *fin,                                              // input file
                          ...
                          vector<vector<Vector4d, Eigen::aligned_allocator<Vector4d> > >
                              &ptt  // point tracks - each vector is <camera_index u v>
) {
  ifstream ifs(fin);
  if (!ifs) {
  ...
  // print some stats
  double nprjs = 0;
  for (int i = 0; i < npts; i++) nprjs += ptt[i].size();
...
void sba::writeSparseA(const char *fname, SysSBA &sba) {
...

  {
    ...
    for (int i = 0; i < nrows; i++)
      for (int j = i; j < ncols; j++) {
        double a = sba.A(i, j);
        if (a != 0.0) ofs << i << " " << j << " " << setprecision(16) << a << endl;
      }
...
int sba::readGraphFile(const char *filename, SysSBA &sbaout) {
  ...
  vector<Vector3d, Eigen::aligned_allocator<Vector3d> > camts;  // cam translation
  vector<Vector3d, Eigen::aligned_allocator<Vector3d> > ptps;   // point position
  vector<vector<Vector11d, Eigen::aligned_allocator<Vector11d> > >
      ptts;  // point tracks - each vector is <camera_index kp_idex u v>
...
int sba::ParseGraphFile(const char *fin,                                              // input file
  ...
  while (getline(ifs, line)) {
    ...
    if (pos != string::npos) continue;  // comment line

    if (type == "VERTEX_SE3" || type == "VERTEX_CAM")  // have a camera node
    {
      int n;
      double tx, ty, tz, qx, qy, qz, qw, fx, fy, cx, cy, bline;
      if (!(ss >> n >> tx >> ty >> tz >> qx >> qy >> qz >> qw >> fx >> fy >> cx >> cy >> bline)) {
    ...
    else if (type == "EDGE_PROJECT_XYZ" || type == "EDGE_PROJECT_P2MC" || type == "EDGE_PROJECT_P2SC")  // have an edge
    {
      int n1, n2;
      ...
      if (type == "EDGE_PROJECT_P2SC") {
        if (!(ss >> n1 >> n2 >> u >> v >> d >> cv0 >> cv1 >> cv2 >> cv3 >> cv4 >> cv5)) {
      ...
      // get point track
      if (ptts.size() < (size_t)pi + 1) ptts.resize(pi + 1);
  ...
  int ncams = camp.size();
  int npts = ptp.size();
  for (int i = 0; i < npts; i++) nprjs += ptts[i].size();
...
int sba::writeGraphFile(const char *filename, SysSBA &sba, bool mono) {
  ...
  for (i = 0; i < (unsigned int)ncams; i++) {
    ...
    // cam params
    outfile << sba.nodes[i].Kcam(0, 0) << ' ' << sba.nodes[i].Kcam(1, 1) << ' ' << sba.nodes[i].Kcam(0, 2) << ' '
            << sba.nodes[i].Kcam(1, 2) << ' ' << sba.nodes[i].baseline << endl;
  }
  ...
  for (i = 0; i < sba.tracks.size(); i++) {
    ...
    // World <x y z>
    outfile << sba.tracks[i].point(0) << ' ' << sba.tracks[i].point(1) << ' ' << sba.tracks[i].point(2) << endl;
    ...
    for (ProjMap::iterator itr = prjs.begin(); itr != prjs.end(); itr++) {
      ...
      if (prj.stereo && !mono) {
        ...
        outfile << ncams + i << ' ' << prj.ndi << ' ' << prj.kp(0) << ' ' << prj.kp(1) << ' ' << prj.kp(2) << ' ';
        outfile << "1 0 0 0 1 1" << endl;  // covariance
      } else {
        outfile << "EDGE_PROJECT_P2MC ";  // mono edge
        outfile << ncams + i << ' ' << prj.ndi << ' ' << prj.kp(0) << ' ' << prj.kp(1) << ' ';
        ...
        prec)  // constraint covariance
{
  ...
  for (int i = 0; i < (int)ctrans.size(); i++) {
    ...
    if ((con.ndr == n && con.nd1 <= n - 1) || (con.nd1 == n && con.ndr <= n - 1)) {
      con.tmean = ctrans[i];
...
int sba::readSPAGraphFile(const char *filename, SysSPA &spaout) {
  ...
  cout << "# [ReadSPAFile] Found " << (int)ntrans.size() << " nodes and " << (int)cind.size() << " constraints" << endl;

  int nnodes = ntrans.size();

  // add in nodes
  for (int i = 0; i < nnodes; i++) addnode(spaout, i, ntrans, nqrot, cind, ctrans, cqrot, prec);
...
int sba::ParseSPAGraphFile(
    const char *fin,                                                                   // input file
    std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> > &ntrans,  // node translation
    std::vector<Eigen::Vector4d, Eigen::aligned_allocator<Eigen::Vector4d> > &nqrot,   // node rotation
    std::vector<Eigen::Vector2i, Eigen::aligned_allocator<Eigen::Vector2i> > &cind,    // constraint indices
    std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d> > &ctrans,  // constraint local translation
        ...
        &prec)  // constraint covariance
{
  ...
  while (getline(ifs, line)) {
    ...
    if (type == "EDGE_SE3_SE3")  // have an edge
    {
      ...
      // covar
      if (!(ss >> cv[0] >> cv[1] >> cv[2] >> cv[3] >> cv[4] >> cv[5] >> cv[6] >> cv[7] >> cv[8] >> cv[9] >> cv[10] >>
            cv[11] >> cv[12] >> cv[13] >> cv[14] >> cv[15] >> cv[16] >> cv[17] >> cv[18] >> cv[19] >> cv[20])) {
        cout << "[ReadSPA] Bad EDGE_SE3_SE3 at line " << nline << endl;
        return -1;
      }
      Matrix<double, 6, 6> m;
