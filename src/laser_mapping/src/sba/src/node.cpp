...
namespace sba {
void Node::setTransform() { transformW2F(w2n, trans, qrot); }

//
// sets incremental angle derivatives
//
void Node::setDri() { setDr(true); }
...
void Node::normRot() {
  // LOG(INFO) << "[NormRot] qrot start = " << qrot.transpose();
  ...
  // LOG(INFO) << "[NormRot] qrot end   = " << qrot.transpose();
}
...
void Node::normRotLocal() {
  ...
  // std::cout << "[NormRot] qrot end   = " << qrot.transpose();
}
...
// transforms
void transformW2F(Eigen::Matrix<double, 3, 4> &m, const Eigen::Matrix<double, 4, 1> &trans,
...
void transformF2W(Eigen::Matrix<double, 3, 4> &m, const Eigen::Matrix<double, 4, 1> &trans,
                  const Eigen::Quaternion<double> &qrot) {
...
void transformN2N(Eigen::Matrix<double, 4, 1> &trans, Eigen::Quaternion<double> &qr, Node &nd0, Node &nd1) {
