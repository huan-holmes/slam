...
Proj::Proj(int ci, Eigen::Vector3d &q, bool stereo)
    : ndi(ci), kp(q), stereo(stereo), isValid(true), useCovar(false), pointPlane(false) {}

Proj::Proj(int ci, Eigen::Vector2d &q)
    : ndi(ci), kp(q(0), q(1), 0), stereo(false), isValid(true), useCovar(false), pointPlane(false) {}

Proj::Proj() : ndi(0), kp(0, 0, 0), stereo(false), isValid(false), useCovar(false), pointPlane(false) {}
...
void Proj::clearCovariance() { useCovar = false; }
...
void Proj::setJacobiansMono_(const Node &nd, const Point &pt, JacobProds *jpp) {
  ...
  for (int i = 0; i < 2; i++)
    for (int j = 0; j < 6; j++)
      if (std::isnan(jacc(i, j))) {
        printf("[SetJac] NaN in jacc(%d,%d)\n", i, j);
        *(int *)0x0 = 0;
      }
#endif
