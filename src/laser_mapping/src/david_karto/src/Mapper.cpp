...
class ScanManager {
 ...
 public:
   ...
   * scan的一组数据集合　起始scan与终点scan的距离必须在这个范围内　集合的数组大小要满足该数组大小　
   * @param pScan
   */
 ...
 * @param pScan
 */
inline void MapperSensorManager::SetLastScan(LocalizedRangeScan *pScan) { GetScanManager(pScan)->SetLastScan(pScan); }
...
void MapperSensorManager::Clear() {
  ...
  forEach(ScanManagerMap, &m_ScanManagers) { delete iter->second; }
...
ScanMatcher *ScanMatcher::Create(Mapper *pMapper, kt_double searchSize, kt_double resolution, kt_double smearDeviation,
  ...
  // calculate search space in grid coordinates 设置搜寻空间的大小 searchSize 真实搜寻的距离0.6m
  ...
  kt_int32s gridSize = searchSpaceSideSize + 2 * pointReadingMargin;  // 设置匹配空间的大小
...
kt_double ScanMatcher::MatchScan(LocalizedRangeScan *pScan, const LocalizedRangeScanVector &rBaseScans, Pose2 &rMean,
  ...
  // 以真实激光坐标为中心 设置 m_pCorrelationGrid 起始点(0,0) 的真实物理坐标
  Vector2<kt_double> offset;
  offset.SetX(scanPose.GetX() -
              (0.5 * (roi.GetWidth() - 1) * m_pCorrelationGrid->GetResolution()));  // 起始点 grid X的真实坐标
  offset.SetY(scanPose.GetY() -
              (0.5 * (roi.GetHeight() - 1) * m_pCorrelationGrid->GetResolution()));  // 起始点 grid Y的真实坐标
  ...
  // compute how far to search in each direction 里程计附近应该搜寻空间的大小 0.3m 默认
  Vector2<kt_double> searchDimensions(m_pSearchSpaceProbs->GetWidth(), m_pSearchSpaceProbs->GetHeight());
  // LOG(INFO)<<"serch width"<<m_pSearchSpaceProbs->GetWidth()
  ...
  kt_double bestResponse = CorrelateScan(
      pScan, scanPose, coarseSearchOffset, coarseSearchResolution, m_pMapper->m_pCoarseSearchAngleOffset->GetValue(),
  ...
  if (m_pMapper->m_pUseResponseExpansion->GetValue() == true) {
    if (math::DoubleEqual(bestResponse, 0.0)) {
#ifdef KARTO_DEBUG
      LOG(INFO) << "Mapper Info: Expanding response search space!";
#endif
      ...
      for (kt_int32u i = 0; i < 3; i++) {
        ...
        bestResponse =
            CorrelateScan(pScan, scanPose, coarseSearchOffset, coarseSearchResolution, newSearchAngleOffset,
                          m_pMapper->m_pCoarseAngleResolution->GetValue(), doPenalize, rMean, rCovariance, false);
...
#ifdef KARTO_DEBUG
      if (math::DoubleEqual(bestResponse, 0.0)) {
        LOG(INFO) << "Mapper Warning: Unable to calculate response!";
      }
#endif
    }
  }
  ...
  if (doRefineMatch) {
    Vector2<kt_double> fineSearchOffset(coarseSearchResolution * 0.5);  // 这里的原始数据应该是不对的
...
#ifdef KARTO_DEBUG
  LOG(INFO) << "  BEST POSE = " << rMean << " BEST RESPONSE = " << bestResponse << ",  VARIANCE = " << rCovariance(0, 0)
            << ", " << rCovariance(1, 1);
#endif
...
kt_double ScanMatcher::CorrelateScan(LocalizedRangeScan *pScan, const Pose2 &rSearchCenter,
  ...
  // setup lookup arrays
  // 记录 末尾激光的相对于激光中心的偏移量位置  m_pGridLookup 一共 searchAngleOffset/searchAngleResolution 组 每一组
  // pScan.size个
  ...
  for (int i = 0; i < yPoses.size(); ++i) {
    ...
    for (int j = 0; j < xPoses.size(); ++j) {
      ...
      for (kt_int32u angleIndex = 0; angleIndex < nAngles; angleIndex++) {
        kt_double angle = startAngle + angleIndex * searchAngleResolution;
        ...
        if (doPenalize && (math::DoubleEqual(response, 0.0) == false)) {
          ...
          // m_pMapper->m_pDistanceVariancePenalty->GetValue() 越大　该值所占的比例越大
          kt_double distancePenalty =
              1.0 - (DISTANCE_PENALTY_GAIN * squaredDistance / m_pMapper->m_pDistanceVariancePenalty->GetValue());
          ...
          kt_double anglePenalty =
              1.0 - (ANGLE_PENALTY_GAIN * squaredAngleDistance / m_pMapper->m_pAngleVariancePenalty->GetValue());
        ...
        pPoseResponse[(i * xPoses.size() + j) * nAngles + angleIndex] =
...
#ifdef KARTO_DEBUG
  LOG(INFO) << "bestPose: " << averagePose;
  LOG(INFO) << "bestResponse: " << bestResponse;
#endif

  if (!doingFineMatch) {
    ComputePositionalCovariance(averagePose, bestResponse, rSearchCenter, rSearchSpaceOffset, rSearchSpaceResolution,
                                searchAngleResolution, rCovariance);
  } else {
    ComputeAngularCovariance(averagePose, bestResponse, rSearchCenter, searchAngleOffset, searchAngleResolution,
                             rCovariance);
  }
...
void ScanMatcher::ComputePositionalCovariance(const Pose2 &rBestPose, kt_double bestResponse,
                                              const Pose2 &rSearchCenter, const Vector2<kt_double> &rSearchSpaceOffset,
  ...
  for (kt_int32u yIndex = 0; yIndex < nY; yIndex++) {
    ...
    for (kt_int32u xIndex = 0; xIndex < nX; xIndex++) {
      ...
      Vector2<kt_int32s> gridPoint =
          m_pSearchSpaceProbs->WorldToGrid(Vector2<kt_double>(rSearchCenter.GetX() + x, rSearchCenter.GetY() + y));
      kt_double response = *(m_pSearchSpaceProbs->GetDataPointer(gridPoint));

      // response is not a low response 得到一系列的response 求均值最后
  ...
  if (norm > KT_TOLERANCE) {
    ...
    // 这里设置了最小协方差　很重要
    kt_double minVarianceXX = 0.1 * math::Square(rSearchSpaceResolution.GetX());  // 最小值默认　0.1*0.005*0.005 0.00025
    ...
    // LOG(INFO)<<<<"bestResponse"<<bestResponse;
    // bestResponse0.819007 varianceXX0.000682962
    // LOG(INFO)<<<<"varianceXX"<<varianceXX;
...
void ScanMatcher::AddScans(const LocalizedRangeScanVector &rScans, Vector2<kt_double> viewPoint) {
  ...
  const_forEach(LocalizedRangeScanVector, &rScans) { AddScan(*iter, viewPoint); }
}
...
kt_double ScanMatcher::GetResponse(kt_int32u angleIndex, kt_int32s gridPositionIndex) const {
  ...
  // add up value for each point 激光中心基准坐标的地址
  kt_int8u *pByte = m_pCorrelationGrid->GetDataPointer() + gridPositionIndex;

  // 查找每个角度下 末尾激光的位置  m_pGridLookup 一共 angleoffset/reasolution 组
  const LookupArray *pOffsets = m_pGridLookup->GetLookupArray(angleIndex);  // 获取每个角度基于激光中心的偏移量
  ...
  for (kt_int32u i = 0; i < nPoints; i++) {
    // ignore points that fall off the grid  pointGridIndex::激光真实的index
    ...
    // 激光中心基准坐标的地址[当前朝向下每个束激光末尾点的偏移量(即位置)]
    // pByte[pAngleIndexPointer[i]] = m_pCorrelationGrid->GetDataPointer()[pointGridIndex]
...
class BreadthFirstTraversal : public GraphTraversal<T> {
 public:
   ...
   */
  BreadthFirstTraversal(Graph<T> *pGraph) : GraphTraversal<T>(pGraph) {}
 ...
 public:
  ...
  virtual std::vector<T *> Traverse(Vertex<T> *pStartVertex, Visitor<T> *pVisitor) {
    ...
    forEach(typename std::vector<Vertex<T> *>, &validVertices) { objects.push_back((*iter)->GetObject()); }
...
MapperGraph::MapperGraph(Mapper *pMapper, kt_double rangeThreshold) : m_pMapper(pMapper) {
  ...
  //
  // mapping_status.node_ind = 0;
  // mapping_status.is_calculating = 0;
  // mapping_status.candidateChain_Size = 0;
  // mapping_status.coarse_response = 0.0;
  // mapping_status.covariance_0_0 = 0.0;
  // mapping_status.covariance_1_1 = 0.0;
  // mapping_status.fine_response = 0.0;
  // mapping_status.coarse_response_threshold = m_pMapper->m_pLoopMatchMinimumResponseCoarse->GetValue();
  // mapping_status.coarse_covariance_threshold = m_pMapper->m_pLoopMatchMaximumVarianceCoarse->GetValue();
  // mapping_status.fine_response_threshold = m_pMapper->m_pLoopMatchMinimumResponseFine->GetValue();
  // mapping_status.loopclosed = 0;
  for (int i = 0; i < 100; i++) {
    slam_point_m_.push_back(-1);
  }
}
...
void MapperGraph::AddEdges(LocalizedRangeScan *pScan, const Matrix3 &rCovariance) {
  // step 1 和上一帧激光链接
  // step 2 和running scan 里面最近的节点 链接
  // step 3 和near scan 里面最近的节点　链接

  MapperSensorManager *pSensorManager = m_pMapper->m_pMapperSensorManager;

  const Name &rSensorName = pScan->GetSensorName();

  // mapping_status.node_ind++;

  // STEP 1 link to previous scan 首先和上一帧激光 link 添加边
  ...
  if (pSensorManager->GetLastScan(rSensorName) == NULL) {
    ...
    // link to running scans 和 running scan 里面最近的节点　添加边
    Pose2 scanPose = pScan->GetSensorPose();
  ...
  // STEP 3 link to other near chains (chains that include new scan are invalid)和附近的chain 进行链接
...
kt_bool MapperGraph::TryCloseLoop(LocalizedRangeScan *pScan, const Name &rSensorName) {
  ...
  // mapping_status.candidateChain_Size = candidateChain.size();
  ...
  while (!candidateChain.empty()) {
    ...
    // bestPose.GetX()) +
    //     (pScan->GetCorrectedPose().GetY() - bestPose.GetY())*(pScan->GetCorrectedPose().GetY() - bestPose.GetY());
    //
    // // 大于1m
    // if (diff_dist > 1.) {
    //   coarseResponse = 0.;
    //   LOG(INFO) << "loop change too large!!!" << diff_dist ;
    // }

    // m_pMapper->processing_mutex_=false;
    // while(m_pMapper->new_laser_in_)
    //{
    // LOG(INFO)<<" 2 handle new laser";
    // usleep(1000*10);
    //}
    ...
    // mapping_status.is_calculating = 0;
    // mapping_status.coarse_response = coarseResponse;
    // mapping_status.covariance_0_0 = covariance(0, 0);
    // mapping_status.covariance_1_1 = covariance(1, 1);
    //
    // if (loopClosed) {
    //   mapping_status.loopclosed++;
    // }

    // mapping_loopclosure_state_pub.publish(mapping_status);
    // LOG(INFO) << coarseResponse << " - " << covariance(0, 0)
    //           << " - " << covariance(1, 1) << " - " << fineResponse
    //           << " - " ;
  }

  // mapping_state_pub_.publish(mapping_status);
  // mapping_status.coarse_response = 0;
  // mapping_status.covariance_0_0 = 0;
  // mapping_status.covariance_1_1 = 0;
  // mapping_status.fine_response = 0;
  // mapping_status.is_calculating = 0;
  return loopClosed;
}
...
Edge<LocalizedRangeScan> *MapperGraph::AddEdge(LocalizedRangeScan *pSourceScan, LocalizedRangeScan *pTargetScan,
                                               kt_bool &rIsNewEdge) {
  ...
  Graph<LocalizedRangeScan>::AddEdge(pEdge);  // m_Edges.push_back(pEdge);
  rIsNewEdge = true;
  return pEdge;
}

void MapperGraph::LinkScans(LocalizedRangeScan *pFromScan, LocalizedRangeScan *pToScan, const Pose2 &rMean,
...
void MapperGraph::LinkChainToScan(const LocalizedRangeScanVector &rChain, LocalizedRangeScan *pScan, const Pose2 &rMean,
                                  const Matrix3 &rCovariance) {
...
std::vector<LocalizedRangeScanVector> MapperGraph::FindNearChains(LocalizedRangeScan *pScan) {
  ...
  const_forEach(LocalizedRangeScanVector, &nearLinkedScans) {
    ...
    kt_int32u end =
        static_cast<kt_int32u>(m_pMapper->m_pMapperSensorManager->GetScans(pNearScan->GetSensorName()).size());
...
void Mapper::InitializeParameters() {
  m_pUseScanMatching = new Parameter<kt_bool>("UseScanMatching",
  ...
  m_pUseScanBarycenter = new Parameter<kt_bool>("UseScanBarycenter",
                                                "Use the barycenter of scan endpoints to define distances between "
                                                "scans.",
                                                true, GetParameterManager());

  m_pUseYawLineComp = new Parameter<kt_bool>("UseYawLineComp", "Use yaw line comp", true, GetParameterManager());
  ...
  m_pMinimumTravelHeading =
      new Parameter<kt_double>("MinimumTravelHeading",
  ...
  m_pScanBufferSize = new Parameter<kt_int32u>("ScanBufferSize",
  ...
  m_pScanBufferMaximumScanDistance =
      new Parameter<kt_double>("ScanBufferMaximumScanDistance",
  ...
  m_pDoLoopClosing =
      new Parameter<kt_bool>("DoLoopClosing", "Enable/disable loop closure.", true, GetParameterManager());
  ...
  m_pLoopMatchMinimumResponseCoarse =
      new Parameter<kt_double>("LoopMatchMinimumResponseCoarse",
                               "If response is larger then this, then initiate loop closure search at "
                               "the coarse resolution.",
                               0.8, GetParameterManager());

  m_pLoopMatchMinimumResponseFine =
      new Parameter<kt_double>("LoopMatchMinimumResponseFine",
                               "If response is larger then this, then initiate loop closure search at "
                               "the fine resolution.",
                               0.8, GetParameterManager());
  ...
  m_pCorrelationSearchSpaceDimension =
      new Parameter<kt_double>("CorrelationSearchSpaceDimension",
  ...
  m_pCorrelationSearchSpaceResolution = new Parameter<kt_double>(
      "CorrelationSearchSpaceResolution", "The resolution (size of a grid cell) of the correlation grid.", 0.01,
      GetParameterManager());
  ...
  m_pLoopSearchSpaceDimension = new Parameter<kt_double>(
      "LoopSearchSpaceDimension", "The size of the search grid used by the matcher.", 8.0, GetParameterManager());

  m_pLoopSearchSpaceResolution = new Parameter<kt_double>(
      "LoopSearchSpaceResolution", "The resolution (size of a grid cell) of the correlation grid.", 0.05,
      GetParameterManager());
  ...
  m_pAngleVariancePenalty = new Parameter<kt_double>("AngleVariancePenalty", "See DistanceVariancePenalty.",
                                                     math::Square(math::DegreesToRadians(20)), GetParameterManager());

  m_pFineSearchAngleOffset =
      new Parameter<kt_double>("FineSearchAngleOffset", "The range of angles to search during a fine search.",
                               math::DegreesToRadians(0.2), GetParameterManager());

  m_pCoarseSearchAngleOffset =
      new Parameter<kt_double>("CoarseSearchAngleOffset", "The range of angles to search during a coarse search.",
                               math::DegreesToRadians(20), GetParameterManager());

  m_pCoarseAngleResolution =
      new Parameter<kt_double>("CoarseAngleResolution", "Resolution of angles to search during a coarse search.",
                               math::DegreesToRadians(2), GetParameterManager());

  m_pMinimumAnglePenalty =
      new Parameter<kt_double>("MinimumAnglePenalty",
  ...
  // when valid laser percentate less than this value the sensor best pose will use odom pose
  validLaserPercentage_ = 0.0;
  // when match reponse less than this value the sensor best pose will use odom pose
  minimumMatchScore_ = 0.0;
}
/* Adding in getters and setters here for easy parameter access */

// General Parameters

bool Mapper::getParamUseScanMatching() { return static_cast<bool>(m_pUseScanMatching->GetValue()); }

bool Mapper::getParamUseScanBarycenter() { return static_cast<bool>(m_pUseScanBarycenter->GetValue()); }

double Mapper::getParamMinimumTimeInterval() { return static_cast<double>(m_pMinimumTimeInterval->GetValue()); }

double Mapper::getParamMinimumTravelDistance() { return static_cast<double>(m_pMinimumTravelDistance->GetValue()); }
...
int Mapper::getParamScanBufferSize() { return static_cast<int>(m_pScanBufferSize->GetValue()); }
...
double Mapper::getParamLinkScanMaximumDistance() { return static_cast<double>(m_pLinkScanMaximumDistance->GetValue()); }

double Mapper::getParamLoopSearchMaximumDistance() {
  return static_cast<double>(m_pLoopSearchMaximumDistance->GetValue());
}

bool Mapper::getParamDoLoopClosing() { return static_cast<bool>(m_pDoLoopClosing->GetValue()); }

int Mapper::getParamLoopMatchMinimumChainSize() { return static_cast<int>(m_pLoopMatchMinimumChainSize->GetValue()); }
...
double Mapper::getParamFineSearchAngleOffset() { return static_cast<double>(m_pFineSearchAngleOffset->GetValue()); }

double Mapper::getParamCoarseSearchAngleOffset() { return static_cast<double>(m_pCoarseSearchAngleOffset->GetValue()); }

double Mapper::getParamCoarseAngleResolution() { return static_cast<double>(m_pCoarseAngleResolution->GetValue()); }

double Mapper::getParamMinimumAnglePenalty() { return static_cast<double>(m_pMinimumAnglePenalty->GetValue()); }

double Mapper::getParamMinimumDistancePenalty() { return static_cast<double>(m_pMinimumDistancePenalty->GetValue()); }

bool Mapper::getParamUseResponseExpansion() { return static_cast<bool>(m_pUseResponseExpansion->GetValue()); }

/* Setters for parameters */
// General Parameters
void Mapper::setParamUseScanMatching(bool b) { m_pUseScanMatching->SetValue((kt_bool)b); }

void Mapper::setParamUseScanBarycenter(bool b) { m_pUseScanBarycenter->SetValue((kt_bool)b); }

void Mapper::setParamUseYawLineComp(bool b) { m_pUseYawLineComp->SetValue((kt_bool)b); }

void Mapper::setParamMinimumTimeInterval(double d) { m_pMinimumTimeInterval->SetValue((kt_double)d); }

void Mapper::setParamMinimumTravelDistance(double d) { m_pMinimumTravelDistance->SetValue((kt_double)d); }

void Mapper::setParamMinimumTravelHeading(double d) { m_pMinimumTravelHeading->SetValue((kt_double)d); }

void Mapper::setParamScanBufferSize(int i) { m_pScanBufferSize->SetValue((kt_int32u)i); }

void Mapper::setParamScanBufferMaximumScanDistance(double d) {
  m_pScanBufferMaximumScanDistance->SetValue((kt_double)d);
}

void Mapper::setParamLinkMatchMinimumResponseFine(double d) { m_pLinkMatchMinimumResponseFine->SetValue((kt_double)d); }

void Mapper::setParamLinkScanMaximumDistance(double d) { m_pLinkScanMaximumDistance->SetValue((kt_double)d); }

void Mapper::setParamLoopSearchMaximumDistance(double d) { m_pLoopSearchMaximumDistance->SetValue((kt_double)d); }

void Mapper::setParamDoLoopClosing(bool b) { m_pDoLoopClosing->SetValue((kt_bool)b); }

void Mapper::setParamLoopMatchMinimumChainSize(int i) { m_pLoopMatchMinimumChainSize->SetValue((kt_int32u)i); }
...
void Mapper::setParamLoopMatchMinimumResponseFine(double d) { m_pLoopMatchMinimumResponseFine->SetValue((kt_double)d); }
...
void Mapper::setParamLoopSearchSpaceDimension(double d) { m_pLoopSearchSpaceDimension->SetValue((kt_double)d); }

void Mapper::setParamLoopSearchSpaceResolution(double d) { m_pLoopSearchSpaceResolution->SetValue((kt_double)d); }
...
void Mapper::setParamAngleVariancePenalty(double d) { m_pAngleVariancePenalty->SetValue((kt_double)math::Square(d)); }

void Mapper::setParamFineSearchAngleOffset(double d) { m_pFineSearchAngleOffset->SetValue((kt_double)d); }

void Mapper::setParamCoarseSearchAngleOffset(double d) { m_pCoarseSearchAngleOffset->SetValue((kt_double)d); }

void Mapper::setParamCoarseAngleResolution(double d) { m_pCoarseAngleResolution->SetValue((kt_double)d); }

void Mapper::setParamMinimumAnglePenalty(double d) { m_pMinimumAnglePenalty->SetValue((kt_double)d); }

void Mapper::setParamMinimumDistancePenalty(double d) { m_pMinimumDistancePenalty->SetValue((kt_double)d); }

void Mapper::setParamUseResponseExpansion(bool b) { m_pUseResponseExpansion->SetValue((kt_bool)b); }

void Mapper::setValidLaserPercent(double per) { validLaserPercentage_ = per; }
void Mapper::setminimumMatchScanScore(double score) { minimumMatchScore_ = score; }
...
void Mapper::Initialize(kt_double rangeThreshold) {
  if (!m_Initialized) {
    ...
    // 使用在loop_closure
    m_pSequentialScanMatcher_loop = ScanMatcher::Create(
        this, m_pCorrelationSearchSpaceDimension->GetValue(), m_pCorrelationSearchSpaceResolution->GetValue(),
        m_pCorrelationSearchSpaceSmearDeviation->GetValue(), rangeThreshold);

    m_pMapperSensorManager =
        new MapperSensorManager(m_pScanBufferSize->GetValue(), m_pScanBufferMaximumScanDistance->GetValue());

    m_pGraph = new MapperGraph(this, rangeThreshold);

    ros::NodeHandle n("~");
    n.param("running_scan", running_scan_, false);
    if (running_scan_) {
      LOG(WARNING) << ("use running scan as submap");
    } else {
      LOG(WARNING) << ("use near scan as submap");
    }
...
void Mapper::TriggerFinishSlam(unsigned int number) {
  LOG(INFO) << "TriggerFinishSlam ......";
  slam_process_end_flag_ = true;
}

kt_bool Mapper::Process(Object * /*pObject*/) { return true; }

LocalizedRangeScanVector Mapper::FindNearScans(LocalizedRangeScan *pScan, const Name &rSensorName,
                                               double rangeThreashold) {
  LocalizedRangeScanVector chain;  // return value

  Pose2 pose = pScan->GetReferencePose(m_pUseScanBarycenter->GetValue());

  auto nScans = static_cast<kt_int32u>(m_pMapperSensorManager->GetScans(rSensorName).size());
  for (int i = 0; i < nScans; i++) {
    LocalizedRangeScan *pCandidateScan = m_pMapperSensorManager->GetScan(rSensorName, i);

    Pose2 candidateScanPose = pCandidateScan->GetReferencePose(m_pUseScanBarycenter->GetValue());

    kt_double squaredDistance = candidateScanPose.GetPosition().SquaredDistance(pose.GetPosition());
    // 使用基本两倍的节点来进行激光匹配矫正　因为会有其他节点的激光打到这组激光的范围区域里面来进来
    if (squaredDistance < math::Square(rangeThreashold * 1.8) + KT_TOLERANCE) {
      chain.push_back(pCandidateScan);
    }
  }
...
kt_bool Mapper::Process(LocalizedRangeScan *pScan, double current_time) {
  static Pose2 last_pose2{0, 0, 0};
  if (pScan != nullptr) {
    karto::LaserRangeFinder *pLaserRangeFinder = pScan->GetLaserRangeFinder();

    // validate scan
    if (pLaserRangeFinder == nullptr || pScan == nullptr || !pLaserRangeFinder->Validate(pScan)) {
      return false;
    }
    ...
    RangeReadingsVector m_pRangeReadings = pScan->GetRangeReadingsVector();
    ...
    if (m_pUseScanMatching->GetValue()) {
      ...
      // 将当前帧激光添加到 running scan 组
      m_pMapperSensorManager->AddRunningScan(pScan);

      auto start_time = ros::WallTime::now();

      if (m_pGraph->manual_loop_closure_flag) {
        LocalizedRangeScan tmpScan(pScan->GetSensorName(), pScan->GetRangeReadingsVector());
        tmpScan.SetUniqueId(pScan->GetUniqueId());
        tmpScan.SetTime(pScan->GetTime());
        tmpScan.SetStateId(pScan->GetStateId());
        tmpScan.SetCorrectedPose(pScan->GetCorrectedPose());
        Pose2 loop_pose{pScan->GetCorrectedPose()};
        loop_pose.SetX(m_pGraph->loop_pos_x);
        loop_pose.SetY(m_pGraph->loop_pos_y);
        tmpScan.SetSensorPose(loop_pose);
        std::cout << "loop_pose: " << loop_pose << std::endl;
 ...
 * @param pListener
 */
void Mapper::AddListener(MapperListener *pListener) { m_Listeners.push_back(pListener); }
...
void Mapper::FireInfo(const std::string &rInfo) const {
  const_forEach(std::vector<MapperListener *>, &m_Listeners) { (*iter)->Info(rInfo); }
}
...
void Mapper::SetScanSolver(ScanSolver *pScanOptimizer) { m_pScanOptimizer = pScanOptimizer; }

MapperGraph *Mapper::GetGraph() const { return m_pGraph; }

ScanMatcher *Mapper::GetSequentialScanMatcher() const { return m_pSequentialScanMatcher; }

ScanMatcher *Mapper::GetLoopScanMatcher() const { return m_pGraph->GetLoopScanMatcher(); }
}  // namespace karto
