...
class LinkInfo : public EdgeLabel {
 ...
 public:
   ...
   * @return first pose
   */
  inline const Pose2& GetPose1() { return m_Pose1; }

  /**
   * Gets the second pose
   * @return second pose
   */
  inline const Pose2& GetPose2() { return m_Pose2; }

  /**
   * Gets the pose difference
   * @return pose difference
   */
  inline const Pose2& GetPoseDifference() { return m_PoseDifference; }

  /**
   * Gets the link covariance
   * @return link covariance
   */
  inline const Matrix3& GetCovariance() { return m_Covariance; }
...
class Vertex {
 ...
 public:
   ...
   * @param pObject
   */
  Vertex(T* pObject) : m_pObject(pObject) {}
 ...
 private:
   ...
   */
  inline void AddEdge(Edge<T>* pEdge) { m_Edges.push_back(pEdge); }
...
class Edge {
 public:
   ...
   * @param pTarget
   */
  Edge(Vertex<T>* pSource, Vertex<T>* pTarget) : m_pSource(pSource), m_pTarget(pTarget), m_pLabel(NULL) {
 ...
 public:
   ...
   */
  inline Vertex<T>* GetSource() const { return m_pSource; }

  /**
   * Gets the target vertex
   * @return target vertex
   */
  inline Vertex<T>* GetTarget() const { return m_pTarget; }

  /**
   * Gets the link info
   * @return link info
   */
  inline EdgeLabel* GetLabel() { return m_pLabel; }

  /**
   * Sets the link payload
   * @param pLabel
   */
  inline void SetLabel(EdgeLabel* pLabel) { m_pLabel = pLabel; }
...
class GraphTraversal {
 public:
   ...
   * @param pGraph
   */
  GraphTraversal(Graph<T>* pGraph) : m_pGraph(pGraph) {}
...
class Graph {
 ...
 public:
   ...
   * @param pVertex
   */
  inline void AddVertex(const Name& rName, Vertex<T>* pVertex) { m_Vertices[rName].push_back(pVertex); }
  ...
  void Clear() {
    forEachAs(typename VertexMap, &m_Vertices, indexIter) {
      // delete each vertex
      forEach(typename std::vector<Vertex<T>*>, &(indexIter->second)) { delete *iter; }
    }
    m_Vertices.clear();

    const_forEach(typename std::vector<Edge<T>*>, &m_Edges) { delete *iter; }
    m_Edges.clear();
  }
   ...
   */
  inline const VertexMap& GetVertices() const { return m_Vertices; }
...
class KARTO_EXPORT MapperGraph : public Graph<LocalizedRangeScan> {
 public:
  ...
  //
  // ros::Publisher mapping_state_pub_;
  //
  // ros::Publisher mapping_loopclosure_state_pub;
  // 手动回环
  boost::atomic_bool manual_loop_closure_flag{false};
  boost::atomic<double> loop_pos_x{0.};
  boost::atomic<double> loop_pos_y{0.};
 ...
 public:
   ...
   */
  Edge<LocalizedRangeScan>* AddEdge(LocalizedRangeScan* pSourceScan, LocalizedRangeScan* pTargetScan,
                                    kt_bool& rIsNewEdge);
   ...
   * @return scan matcher
   */
  inline ScanMatcher* GetLoopScanMatcher() const { return m_pLoopScanMatcher; }

  // void checkAprilTag(const apriltags::AprilTagDetections& detections,float id);
 ...
 private:
   ...
   * @param rCovariance
   */
  void LinkScans(LocalizedRangeScan* pFromScan, LocalizedRangeScan* pToScan, const Pose2& rMean,
   ...
   * @param rCovariance
   */
  void LinkChainToScan(const LocalizedRangeScanVector& rChain, LocalizedRangeScan* pScan, const Pose2& rMean,
                       const Matrix3& rCovariance);
   ...
   */
  LocalizedRangeScanVector FindPossibleLoopClosure(LocalizedRangeScan* pScan, const Name& rSensorName,
                                                   kt_int32u& rStartNum);
  /**
...
class CorrelationGrid : public Grid<kt_int8u> {
 ...
 public:
   ...
   */
  static CorrelationGrid* CreateGrid(kt_int32s width, kt_int32s height, kt_double resolution,
   ...
   */
  inline const Rectangle2<kt_int32s>& GetROI() const { return m_Roi; }

  /**
   * Sets the Region Of Interest (ROI)
   * @param roi
   */
  inline void SetROI(const Rectangle2<kt_int32s>& roi) { m_Roi = roi; }
...
class KARTO_EXPORT ScanMatcher {
 ...
 public:
  ...
  void ComputePositionalCovariance(const Pose2& rBestPose, kt_double bestResponse, const Pose2& rSearchCenter,
                                   const Vector2<kt_double>& rSearchSpaceOffset,
                                   const Vector2<kt_double>& rSearchSpaceResolution, kt_double searchAngleResolution,
                                   Matrix3& rCovariance);
   ...
   * @return correlation grid
   */
  inline CorrelationGrid* GetCorrelationGrid() const { return m_pCorrelationGrid; }
 ...
 private:
  ...
  boost::shared_mutex shared_boost_mutex_;
  boost::mutex boost_mutex_;

  bool operate_flag_;
  // boost::unique_lock unique_lock_m_;
...
class KARTO_EXPORT MapperSensorManager  // : public SensorManager
{
 ...
 public:
   ...
   * Destructor
   */
  virtual ~MapperSensorManager() { Clear(); }
 ...
 public:
  ...
  inline std::vector<Name> GetSensorNames() {
    ...
    const_forEach(ScanManagerMap, &m_ScanManagers) { deviceNames.push_back(iter->first); }
  ...
  inline kt_int32s getCurrentUniqueID(void) { return m_NextScanId - 1; }
  /**
   ...
   */
  LocalizedRangeScanVector& GetScans(const Name& rSensorNaame);
 ...
 private:
   ...
   * @return ScanManager
   */
  inline ScanManager* GetScanManager(LocalizedRangeScan* pScan) { return GetScanManager(pScan->GetSensorName()); }
...
class KARTO_EXPORT Mapper : public Module {
 ...
 public:
  ...
  void TriggerFinishSlam(unsigned int number);
  /**
   * Resets the mapper.
   ...
   */
  virtual kt_bool Process(LocalizedRangeScan* pScan, double current_time);
   ...
   */
  virtual LocalizedRangeScanVector GetAllProcessedScans() const;
   ...
   */
  inline MapperSensorManager* GetMapperSensorManager() const { return m_pMapperSensorManager; }
 ...
 private:
   ...
   * @param pScan scan to be checkedag
  ...
  bool slam_process_end_flag_;
  std::vector<int> slam_point;
 ...
 private:
   ...
   * @return chain that can possibly close a loop with given scan
   */
  LocalizedRangeScanVector FindNearScans(LocalizedRangeScan* pScan, const Name& rSensorName, double rangeThreashold);
 ...
 private:
  ...
  kt_bool running_scan_;
  ScanMatcher* m_pSequentialScanMatcher;

  ScanMatcher* m_pSequentialScanMatcher_loop;
  ...
  double validLaserPercentage_;
  double minimumMatchScore_;
  // std::queue<LocalizedRangeScan> temp_LocalizedRangeScan_;
  ...
  Parameter<kt_bool>* m_pUseYawLineComp;
 ...
 public:
  ...
  void setParamUseYawLineComp(bool b);
  ...
  void setminimumMatchScanScore(double score);
  void setValidLaserPercent(double per);
};
}  // namespace karto
